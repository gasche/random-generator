...
(** Represents generator that may fail. It is quite common when doing
    random testing to ask for an element randomly generated by [gen],
    but only one that satisfies some predicate [p]. This can be
    expressed simply as [succeed gen |> guard p |> backtrack].
*)
...
{[
    ...
    a function that, given a given amount of fuel, will produce an 'a
    ...
    with only two branches, you can use [split_int : int -> (int * int)]).

    The [Fuel] submodule provides the basic operations on fueled
    generators.  We also provide some convenience functions outside
...
{[
    ...
    generator of *random boolean trees* of a given shape (decided by
